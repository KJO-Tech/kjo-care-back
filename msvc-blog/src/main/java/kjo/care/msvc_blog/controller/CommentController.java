...
public class CommentController {
    ...
    @GetMapping("/all")
    public ResponseEntity<ApiResponseDto<List<CommentResponseDto>>> findAll() {
        List<CommentResponseDto> response = commentService.findAllComments();
        return ResponseBuilder.buildResponse(HttpStatus.OK, "Comentarios obtenidos correctamente", true, response);
    }
    ...
    @PostMapping()
    public ResponseEntity<ApiResponseDto<CommentResponseDto>> create(@ModelAttribute @Validated CommentRequestDto comment, @AuthenticationPrincipal Jwt jwt) {
        String userId = jwt.getSubject();
        CommentResponseDto createComment = commentService.saveComment(comment, userId);
        return ResponseBuilder.buildResponse(HttpStatus.CREATED, "Comentario creado correctamente", true, createComment);
    }
    ...
    public ResponseEntity<ApiResponseDto<CommentResponseDto>> update(@PathVariable UUID id, @ModelAttribute @Validated CommentRequestDto comment, @AuthenticationPrincipal Jwt jwt) {
        String userId = jwt.getSubject();
        CommentResponseDto updated = commentService.updateComment(id, comment,userId);
        return ResponseBuilder.buildResponse(HttpStatus.OK, "Comentario actualizado correctamente", true, updated);
    }
    ...
    public ResponseEntity<ApiResponseDto<Object>> delete(@PathVariable UUID id,  @AuthenticationPrincipal Jwt jwt) {
        ...
        return ResponseBuilder.buildResponse(HttpStatus.OK, "Comentario eliminado correctamente", true, null);
    }
}
